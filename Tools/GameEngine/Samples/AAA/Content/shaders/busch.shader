...
void main( void )
{
	...
	float sichtbar = max(0.0, dot(viewerPos, tsbNormal));
	// Wenn normale von Kamera wegzeigt dreht man sie einfach um
	if(sichtbar < 0.0)
	{
		tsbNormal = tsbNormal * (-1.0);
	}
	
	
	// Bewegung im Wind
	pos.y = pos.y - sin(pos.y * 3 + counter.a * 0.02) * 0.02;
	pos.x = pos.x - cos(pos.x * 3 + counter.a * 0.04) * 0.02;
	pos.z = pos.z - sin(pos.z * 3 + counter.a * 0.02) * 0.02;
...
void main( void )
{
	...
	vec3 tempColor = calcPhongSpotLight( newPos, normalize( normal ), albedo.rgb, specParams.x, specParams.y, -vsPos.z, 0.3 ) * 0.7;
	gl_FragColor.rgb = ((albedo.rgb * 0.2) + (tempColor * 0.8));
}
...
void main( void )
{
	...
	gl_FragColor.rgb = albedo.rgb * textureCube( ambientMap, normal ).rgb * 0.4;
