...
void OpenGLWindow::paintGL()
{
	...
	if (m_active)
	{
		if (m_mouseCB && mouseLook)
		{
			POINT cPos;
			GetCursorPos( &cPos );
			SetCursorPos(m_windowWidth / 2, m_windowHeight / 2);	
			m_mouseCB( (float)(cPos.x) - float (m_windowWidth / 2), float (m_windowHeight / 2) - (float)cPos.y, m_userData);
		}
	}
}
...
{    
	...
	case WM_LBUTTONUP:
		...
		if (m_mouseButtonCB) m_mouseButtonCB( (float)cPos.x / (float)m_windowWidth , (float)(m_windowHeight-cPos.y)/ (float)m_windowHeight, m_userData);
		return 0;
	case WM_RBUTTONDOWN:
		mouseLook = true;
		ShowCursor( false );
