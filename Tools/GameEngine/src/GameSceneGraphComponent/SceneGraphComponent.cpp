...
SceneGraphComponent::SceneGraphComponent( GameEntity* owner) : GameComponent(owner, "Horde3D"), 
	m_hordeID(0), m_visibilityFlag(0), m_terrainGeoRes(0), m_applyTrajection(false), m_maxTrajection((float)HUGE_VAL)
{
...
void SceneGraphComponent::setSerializedState(const char *state, size_t length)
{
	...
	if ((s != ls) || (r != lr) || (t != lt))
	{
		if ((t - lt).length() > m_maxTrajection) {
			m_traject_translation.x = 0;
			m_traject_translation.y = 0;
			m_traject_translation.z = 0;
			m_traject_rotation.x = 0;
			m_traject_rotation.y = 0;
			m_traject_rotation.z = 0;
			m_traject_scale.x = 0;
			m_traject_scale.y = 0;
			m_traject_scale.z = 0;
		} else {
		...
		m_applyTrajection = true;
	} else
		m_applyTrajection = false;
}

void SceneGraphComponent::traject()
{
	// apply trajections
	if (m_applyTrajection) {
