...
void Orientation_L::execute_alphaAction(unsigned int myEntityID, unsigned int otherEntityID)
{
	...
	h3dSetNodeTransform( agent_h3d, pos.x, pos.y, pos.z, rot.x, rot.y + 45.0f, rot.z, scale.x, scale.y, scale.z);*/
	...
	h3dGetNodeTransMats( (H3DNode)agent1_h3d, &relArray1, 0 );
	h3dGetNodeTransMats( (H3DNode)agent2_h3d, &relArray2, 0 );
	...
	relMat2.rotate(0, (-1) * Math::Pi/4.0f, 0);
	relMat2.translate(pos2.x, -pos2.y, pos2.z);
	...
	/*h3dSetNodeTransform( agent1_h3d, t.x, t.y, t.z, 
										radToDeg(rot2.x), radToDeg(rot2.y)+90.0f, radToDeg(rot2.z), 
										scale1.x, scale1.y, scale1.z);*/

	//rotation
	rot2.x = radToDeg(rot2.x);
	rot2.y = radToDeg(rot2.y) +90.0f;
...
void Orientation_L::execute_betaAction(unsigned int myEntityID, unsigned int otherEntityID)
{
	/*turnTowards(myEntityID, otherEntityID);
