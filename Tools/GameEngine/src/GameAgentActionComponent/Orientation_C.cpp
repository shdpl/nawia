...
void Orientation_C::execute_alphaAction(unsigned int myEntityID, unsigned int otherEntityID)
{
	...
	h3dSetNodeTransform( agent_h3d, pos.x, pos.y, pos.z, rot.x, rot.y +60.0f, rot.z, scale.x, scale.y, scale.z);*/
	...
	relMat2.rotate(0, (-1)* Math::Pi/3.0f, 0);
	relMat2.translate(pos2.x, -pos2.y, pos2.z);
	...
	/*h3dSetNodeTransform( agent1_h3d, t.x, t.y, t.z, 
										radToDeg(rot2.x), radToDeg(rot2.y)+45.0f, radToDeg(rot2.z), 
										scale1.x, scale1.y, scale1.z);*/
	//rotation
	rot2.x = radToDeg(rot2.x);
	rot2.y = radToDeg(rot2.y) +45.0f;
...
void Orientation_C::execute_betaAction(unsigned int myEntityID, unsigned int otherEntityID)
{
	/*turnTowards(myEntityID, otherEntityID);
