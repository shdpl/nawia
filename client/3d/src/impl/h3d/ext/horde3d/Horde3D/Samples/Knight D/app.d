...
private:
	...
	void keyHandler()
	{
		...
		{
			// Move forward
			_x -= sin( degToRad( _ry ) ) * cos( -degToRad( _rx ) ) * curVel;
			_y -= sin( -degToRad( _rx ) ) * curVel;
			_z -= cos( degToRad( _ry ) ) * cos( -degToRad( _rx ) ) * curVel;
		}
		...
		{
			// Strafe left
			_x += sin( degToRad( _ry - 90) ) * curVel;
			_z += cos( degToRad( _ry - 90 ) ) * curVel;
		}
...
public:
	...
	this()
	{
		for(uint i = 0; i < 320; ++i ) _keys[i] = false;
	...
	void mainLoop( float fps )
	{
		...
		if( !_freeze )
		{
			...
			uint cnt = Horde3D.findNodes( _particleSys, "", SceneNodeTypes.Emitter );
		...
		if( _showFPS )
		{
			...
			if( _timer > 0.3f )
			{
				version(Tango)
				{
					_fpsText = "FPS: " ~ Float.format(new char[32], _curFPS, 2);
				}
				else
				{
					static assert(0);
			...
			// Show text
			Horde3DUtils.showText( _fpsText.ptr, 0, 0.95f, 0.03f, 0, _fontMatRes );

			char[] text;
			version(Tango)
			{
				text= "Weigth: " ~ Float.format(new char[32], _weight);
			}
			else
			{
				static assert(0);
			...
			text ~= "\0";
			Horde3DUtils.showText( text.ptr, 0, 0.91f, 0.03f, 0, _fontMatRes );
		}
	...
	void resize( int width, int height )
	{
		...
		Horde3D.setupCameraView( _cam, 45.0f, cast(float)width / height, 0.1f, 1000.0f );
	}
