...
namespace { // Private.
	...
	static bool LoadPSD( QDataStream & s, const PsdHeader & psd, QImage &img )
	{
		int channel;
		quint32 i, count;
		quint8 len;
		...
		// Create image.
		img = QImage( out, psd.width, psd.height, QImage::Format_ARGB32 ).rgbSwapped();
		delete[] out;
		return true;
	}
